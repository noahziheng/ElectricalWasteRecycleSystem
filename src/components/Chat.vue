...
export default {
  ...
  ready () {
    ...
    if (this.$parent.image === '') {
      ...
      window.Bmob.Image.thumbnail({'image': this.$parent.image, 'mode': 0, 'quality': 100, 'width': 100}).then((obj) => {
        window.userobj.image = 'http://file.bmob.cn/' + obj.url
      })
    }
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (tmp.from.id === this.from.id && tmp.to.id === this.to.id) {
            if (object.get('read') === false) {
              ...
              object.save()
              window.userobj.increment('unread', -1)
              window.userobj.save()
              this.$parent.unread = this.$parent.unread - 1 + ''
              if (this.$parent.unread === '0') {
                this.$parent.unread = ''
              }
            }
            tmp.type = 'other'
    ...
    this.$watch('$parent.unread', function (newVal, oldVal) {
      if (newVal + 0 > oldVal + 0) {
        let Chat = window.Bmob.Object.extend('chat')
        let newquery = new window.Bmob.Query(Chat)
        newquery.equalTo('from', this.from)
        newquery.equalTo('to', this.to)
        newquery.equalTo('read', false)
